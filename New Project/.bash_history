...
{     int left_sum = -999999;     int right_sum = -999999;     int sum = 0;     int i;          static int x = 0;     printf("hi [%d]\n", x++);
...
{     int mid = (high - low)/2;
    ...
    if (low == high);         return a[low];          left_sum = findMaxSubArray(a, low, mid, &left_low, &left_high);
...
int main()
{     int maxSum, low, high;     //int a[] = {13, -3, -25, 20, -3, -16, -23, 18, 20, -7, 12, -5, -22, 15, -4, 7};     int a[] = {1};     maxSum = findMaxSubArray(a, 0, 16, &low, &high);
